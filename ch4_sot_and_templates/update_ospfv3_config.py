...
# -*- coding: utf-8 -*-
"""
Python sample script for updating global OSPFv3 configuration with RESTCONF.
...
def render_template(ospfv3_ids:list, template_name:str):
    with open(template_name, encoding="utf-8") as my_template:
        template = jinja2.Template(my_template.read())
    configuration = template.render(ospfv3=ospfv3_ids)
    print(f"Configuration to be sent: \n {configuration}")
    return configuration

def configure_ospfv3(device_ip:str, username:str, password:str,
                        configuration:list,
                        port:int=443, verify:bool=False)->None:
    '''
    Function to configure ospfv3 on an IOS XE device using RESTCONF.
    '''
    print(f"Configuring ospfv3 on {device_ip}...", end=" ")

    url = f"https://{device_ip}:{port}/restconf/data/Cisco-IOS-XE-native:native/ipv6"
    header = {"Content-Type": "application/yang-data+json"}
    payload = render_template(ospfv3_ids=configuration, template_name="ospfv3.j2")
...
def main():
    ...
    print("# Enabling Ospfv3 for devices")
    for device, config in devices_config.items():
        if config.get("ipv6").get("ospfv3"):
            print("\n#######\n")
            configure_ospfv3(
                device_ip=devices[device],
                username=credentials["username"],
                password=credentials["password"],
                configuration=config["ipv6"]["ospfv3"]["id"]
            )
